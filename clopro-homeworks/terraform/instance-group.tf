...
resource "yandex_compute_instance_group" "ig-1" {
    ...
    instance_template {
        ...
        metadata = {
            ssh-keys   = "ubuntu:${file("/home/srg/.ssh/id_rsa.pub")}"
            user-data  = <<EOF
#!/bin/bash
apt install httpd -y
cd /var/www/html
echo '<html><img src="http://${yandex_storage_bucket.netology-bucket.bucket_domain_name}/458abb04cf5593a4e7dda9819bcc219e.jpg"/></html>' > index.html
service httpd start
EOF
      }
   }
